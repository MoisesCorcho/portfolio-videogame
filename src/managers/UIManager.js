...
import { EVENTS, INTERACTION_TYPES } from '../utils/Constants';
...
export default class UIManager {
  ...
  initListeners() {
    ...
    this.closeModal.addEventListener('click', () => {
      this.hideModal();
      // Resume game using the active scene reference if possible, or key
      this.game.scene.resume('PlayScene');
    });

    // Open Modal Logic (Custom Event)
    window.addEventListener(EVENTS.OPEN_MODAL, (event) => {
  ...
  generateContent(type) {
    if (type === INTERACTION_TYPES.PROFILE) {
    ...
    if (type === INTERACTION_TYPES.EXPERIENCE) {
      let content = `<h2>Experience</h2><div class="space-y-4">`;
    ...
    if (type === INTERACTION_TYPES.SKILLS) {
    ...
    if (type === INTERACTION_TYPES.EDUCATION) {
      let content = `<h2>Education & Certifications</h2><div class="space-y-4">`;
    ...
    return `<p>Unknown section type: ${type}</p>`;
  }
}
